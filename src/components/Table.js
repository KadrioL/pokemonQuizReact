    ...
    resultsRender.push(
      <tr key={pokemonData[i].pkdxnr}>
      {
        pokemonData.slice(i, i + 3)
          .map(pokemon => <td key={pokemon.pkdxnr} className="col-md-4"><b>{pokemon.pkdxnr}:</b><i id={pokemon.pkdxnr} hidden={true}> {capitalizeFirstLetter(pokemon.name)} <img src={pokemon.sprite} alt={pokemon.name} /></i></td>)
      }
      </tr>
    );
  }
